...
positions_df = pd.read_csv('positions.csv', index_col=0,header=0)

# positions_df.loc[positions_df.iloc[:, 0] == 1, 'Atom'] = 'Li'
# positions_df.loc[positions_df.iloc[:, 0] == 2, 'Atom'] = 'e'

progress = open('progress_step3_'+str(from_point)+"_"+str(to_point), 'w')
BX, BY, BZ = [], [], []
x = []
y = []
...
for i in l:
    for j in range(-100, 100):
        x += [i]
        y += [j]
        B = GSOI_single_point(i, j, 0, positions_df, lookup_table)
        BX += [B[0]]
        BY += [B[1]]
        BZ += [B[2]]
        progress.write(str(i)+' ' + str(j)+' ' +
                       str(B[0])+' '+str(B[1]) + ' '+str(B[2])+'\n')
        progress.flush()
progress.close()

B_df = pd.DataFrame({"x": x, "y": y, "BX": BX, "BY": BY, "BZ": BZ})
B_df.to_csv('Bxy_'+str(from_point)+"_"+str(to_point)+".csv")
